...
# Define the period of time that we want analyse
START_DATE = datetime.strptime("2017-04-17 17:45:00", FMT)
END_DATE = datetime.strptime("2017-04-17   20:00:00", FMT)
    ...
    with open(path + name_input) as f:
        ...
        for line in reversed(f.readlines()):
            if line.split(",")[2] == "Linux/UNIX" and line.split(",")[1] == type and line.split(",")[3] == "us-west-1c":
                ...
                date = time_stemp.split("+")[0]

                date_strptime = datetime.strptime(date, FMT)

                # check if the timestamp is in the
                # selected  period
                if START_DATE < date_strptime < END_DATE:
                    ...
                    output += date + "," + str(second) + "," + vm_type + "," + price
